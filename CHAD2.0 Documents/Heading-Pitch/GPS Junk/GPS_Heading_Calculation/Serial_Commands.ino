...
void readCommand(String commandInput) {
  ...
  Serial.println("Command WO markers: " + String(commandWoMarkers));
  ...
  */
  if ( commandInput.startsWith("TARGETLAT") ) {           //command is to set the latitiude
    ...
    ubloxLatitude = commandInput.toFloat(); //set the remaining numbers trailing the command to latitude
    ...
    commandInput.remove(0, 3); //remove the preceing command
    ubloxLongitude = commandInput.toFloat(); //set the remaining numbers trailing the command to latitude
    ...
    commandInput.remove(0, 3); //remove the preceing command
    ubloxAltitude = commandInput.toFloat(); //set the remaining numbers trailing the command to latitude
